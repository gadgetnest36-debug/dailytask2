...
(function () {
  ...
  // Modal manager
  // =========================================================
  // Keeps only one modal active at a time and makes every modal
  // button clickable even when another modal was previously open.
  function openModal(overlay) {
    if (!overlay) return;
    [el.editModalOverlay, el.deleteModalOverlay, el.historyOverlay].forEach((m) => {
      if (m && m !== overlay) {
        m.hidden = true;
  ...
  function closeModal(overlay) {
    ...
    const anyOpen = [el.editModalOverlay, el.deleteModalOverlay, el.historyOverlay]
      .some((m) => m && !m.hidden);
  ...
  function openEditModal(id) {
    // Never allow multiple modal layers to sit on top of each other.
    closeHistory();
    closeDeleteModal();
  ...
  // Global fallback for modal controls. This makes the controls work
  // even if a browser/device has trouble with a nested overlay click.
  document.addEventListener("click", (e) => {
    const target = e.target.closest("button, [role=button]");
    if (!target) return;

    if (target.id === "closeHistoryBtn") {
      e.preventDefault();
      e.stopPropagation();
      closeHistory();
      return;
    }
    if (target.id === "cancelEditBtn") {
      e.preventDefault();
      e.stopPropagation();
      closeEditModal();
      return;
  ...
  function openHistory() {
    // Close any other modal first. This prevents the History overlay from
    // covering the Edit Task buttons and making Cancel appear unclickable.
    closeEditModal();
    closeDeleteModal();
    ...
    openModal(el.historyOverlay);
  }

  function closeHistory() {
    closeModal(el.historyOverlay);
  }
